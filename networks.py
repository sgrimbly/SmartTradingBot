...
class QNetwork(tf.Module):
    ...
    def __init__(self, 
        ...
        self._model.add(Dense(units=self._hidden_layer_sizes[0], 
                        input_dim=self._state_dim, 
                        activation=self._activation))

        for i in range(2,len(self._hidden_layer_sizes)):
            self._model.add(Dense(self._hidden_layer_sizes[i], activation=self._activation))

        self._model.add(Dense(self._action_dim))

    def __call__(self,states) -> tf.Tensor:
